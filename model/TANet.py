...
class TANet(nn.Module):
    ...
    def __init__(self, encoder_arch, local_kernel_size, stride, padding, groups, drtam, refinement, layer_lst=None):
        ...
        self.relu = nn.ReLU(inplace=True)
        self.conv = nn.Conv2d(768, 128, 1, 1, 0, bias=False)

    def forward(self, img):

        img_t0,img_t1 = torch.split(img,3,1)
        if self.encoder_arch == 'sam':
            _, features_t0 = self.encoder1(img_t0)
            _, features_t1 = self.encoder1(img_t1)    
            
            for i, (t0_fmap, t1_fmap) in enumerate(zip(features_t0, features_t1)):
                if i != 12:
            ...
            for i, (f0, f1) in enumerate(zip(t0_ , t1_)):
                f0 = F.interpolate(f0, (int(64*(4/2**i))  , int(64*(4/2**i))), mode='bilinear')
                f1 = F.interpolate(f1, (int(64*(4/2**i))  , int(64*(4/2**i))), mode='bilinear')
                f0 = self.conv(f0)
                f1 = self.conv(f1)
                t0__.append(f0)
                t1__.append(f1)
            ...
            t0 = self.encoder1(img_t0)
            t1 = self.encoder2(img_t1)
...
def dr_tanet_refine_sam(args):
    ...
    drtam = True
    refinement = True
    model = TANet('sam', local_kernel_size, attn_stride,
                    attn_padding, attn_groups, drtam, refinement, args.layers)
    return model
