...
class Model(nn.Module):
    def __init__(self, inc, outc, corr=True, pretrained=True):
        ...
        if self.corr is True:
            self.dec_corr2 = FunctionCorrelation
            self.dec_res2_2 = ResBL( 128, 128, 128, upscale=1, skip1=256+9*9)
        else:
        ...
        if self.corr is True:
            self.dec_corr1 = FunctionCorrelation
            self.dec_conv1 = nn.Conv2d(192+9*9, 64, 7, padding=3, stride=1, bias=False)
        else:
    ...
    def forward(self, x):
        ...
        enc1_f4 = self.enc1_res3_2(enc1_f4) # (b,256,14,14)
        enc1_f5 = self.enc1_res4_1(enc1_f4) # (b,512,7,7)
        enc1_f5 = self.enc1_res4_2(enc1_f5) # (b,512,7,7)
        ...
        enc2_f5 = self.enc2_res4_2(enc2_f5)  # (b,512,7,7)
        ...
        dec = self.unpool(dec) # (b,512,8,8)
        dec = self.dec_bn5(dec)
        dec = self.relu(dec)
        skp = torch.cat([enc1_f5, enc2_f5], 1) # (b,1024,7,7)
        dec = self.dec_res4_2(dec, skip2=skp) # error
...
def cdresnet(args):
    model = Model(inc=6, outc=2, corr=False)
    return model

def cscdnet(args):
    model = Model(inc=6, outc=2, corr=True)
    return model