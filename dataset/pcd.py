...
from dataset.dataset import SCDDataset, get_transforms
import dataset.transforms as T

import dataset.path_config as Data_path

class PCD_CV(SCDDataset):
...
class PCD_Raw_total(SCDDataset):
    # all images are 224x1024
    def __init__(self, root, train=True, transforms=None, revert_transforms=None):
        super(PCD_Raw_total, self).__init__(root, transforms)
        self.root = root
        self.istrain = train
        self.gt, self.t0, self.t1 = self._init_data_list()
        self._transforms = transforms
...
def get_pcd_raw_total(args, sub, train=True):
    assert sub in ['GSV', 'TSUNAMI','PSCD']
    root = os.path.join(Data_path.get_dataset_path('PCD_raw'), sub)
    input_size = args.input_size
    # this function is for only testing PSCD and TSUNAMI (256X1024)
    size_dict = {
        224: (256, 1024),
        256: (256, 1024),
        448: (448, 2048),
    ...
    assert input_size in size_dict, "input_size: {}".format(size_dict.keys())
    transforms, revert_transforms = get_transforms(args, train, size_dict)
    dataset = PCD_Raw_total(root, train, transforms=transforms, revert_transforms=revert_transforms)
    dataset.name = sub
    mode = "Train" if train else "Test"
    print("Domain_{} {}: {}".format(sub, mode, len(dataset)))
    return dataset
...
def get_PSCD(args, train=True):
    return get_pcd_raw(args, 'PSCD', args.data_cv, train=train)

def get_TSUNAMI_total(args, train=True):
    return get_pcd_raw_total(args, 'TSUNAMI', train=train)

def get_PSCD_total(args, train=True):
    return get_pcd_raw_total(args, 'PSCD', train=train)
