...
def proc_target(target, func, **kargs):
    if isinstance(target, list) or isinstance(target, tuple):
        target = [func(tar, **kargs) for tar in target]
    else:
        target = func(target, **kargs)
    return target
...
class RandomHorizontalFlip(object):
    ...
    def __call__(self, image, target):
        if random.random() < self.flip_prob:
            ...
            target = proc_target(target, F.hflip)
        return image, target
...
class RandomCrop(object):
    ...
    def __call__(self, image, target):
        ...
        if isinstance(target, list):
            crop_params = T.RandomCrop.get_params(target[0], (self.size, self.size))
        else:
        ...
        if isinstance(target, list):
            target = [F.crop(tar, *crop_params) for tar in target]
        else:
...
class ToTensor(object):
    def __call__(self, image, target):
        ...
        if isinstance(target, list):
           target_0 = (F.to_tensor(target[0]) != 0).long()
           target_1 = (F.to_tensor(target[1]) != 0).long()
